...
func (s *Store) DrainEmbeddingJobs(ctx context.Context, provider embed.Provider, opts EmbeddingDrainOptions) (EmbeddingDrainStats, error) {
	...
	jobs, err := s.pendingEmbeddingJobs(ctx, opts.Limit)
	if err != nil {
		return stats, err
	}
...
func (s *Store) pendingEmbeddingJobs(ctx context.Context, limit int) ([]embeddingJob, error) {
...
func (s *Store) processEmbeddingBatch(ctx context.Context, provider embed.Provider, opts EmbeddingDrainOptions, jobs []embeddingJob, stats *EmbeddingDrainStats) (bool, error) {
	if err := s.lockEmbeddingJobs(ctx, jobs, opts.Now().Format(timeLayout)); err != nil {
		return false, err
	}
...
func (s *Store) lockEmbeddingJobs(ctx context.Context, jobs []embeddingJob, lockedAt string) error {
	if len(jobs) == 0 {
		return nil
	}
	...
	for _, job := range jobs {
		if _, err := tx.ExecContext(ctx, `
			...
			where message_id = ?
		`, lockedAt, lockedAt, job.MessageID); err != nil {
			return err
		}
	}
	return tx.Commit()
